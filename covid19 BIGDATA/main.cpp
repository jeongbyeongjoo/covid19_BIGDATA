/* [평가항목 1] : 과제 설명
	...
	2022.06.01까지의 코로나바이러스 확진 환자 발생 현황이 기록된 엑셀 자료를 다운로드하여 텍스트 자료로 변환한 후 텍스트 자료를 바탕으로 사
...
int main(void)
{
	...
	if ((fp = fopen("코로나바이러스감염증-19_확진환자_발생현황_220601.txt", "r")) == NULL) // [평가항목 7] : 파일 입출력
	{
	...
	for (int i = 0; feof(fp) == NULL; i++) //텍스트 파일 구조체 배열에 담기
	{
		...
		token = strtok(NULL, " \t\n");
		strcpy(data[i].total, token);
		token = strtok(NULL, " \t\n");
		strcpy(data[i].domestic, token);
		token = strtok(NULL, " \t\n");
		strcpy(data[i].overseas, token);
		token = strtok(NULL, " \t\n");
		strcpy(data[i].death, token);
		*p += 1;
	}
